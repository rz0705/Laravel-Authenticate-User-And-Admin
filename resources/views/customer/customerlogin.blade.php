...
@if (Session('success'))
    <div id="customer-register-message"
        ...
        role="alert">
        <span class="font-medium">{{ session('success') }}</span><a onclick = "customerregister()" href=""
...
@if (Session('warning'))
    <div id="customer-register-message"
        ...
        <span class="font-medium">{{ session('warning') }}</span><a onclick = "customerregister()" href=""
            id="dismiss" class="absolute top-0 right-0 px-3 py-1 cursor-pointer pt-2 text-xl">x</a>
    </div>
@endif
